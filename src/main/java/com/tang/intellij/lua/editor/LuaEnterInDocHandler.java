...
public class LuaEnterInDocHandler implements EnterHandlerDelegate {
    ...
    public Result preprocessEnter(@NotNull PsiFile psiFile, @NotNull Editor editor, @NotNull Ref<Integer> caretOffsetRef,
        ...
        if (comment != null && caretOffset > comment.getTextOffset()) {
            var children = comment.getNode().getChildren(TokenSet.create(LuaDocTypes.DASHES));
            var last = children[children.length - 1];
            if (caretOffset > last.getStartOffset() && comment.getOwner() == null) return null;
            ...
            var textRange = comment.getTextRange();
